		...
		<div id="wrap" class="contatiner login_Wrap"role="main">
			<section class="login_section_wrap clearfix">
				<div class="login_text_box">
					...
					<h1 class="login_logo"><a href="/" class=""><img src="${properties.resourcesPath}/images/header_logo_1.svg" alt="썸트렌드 로고"></a></h1>
				...
				<div class="login_form_box">
					<#if realm.password>
						<form class="login_frm posiRelative" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
							...
							<div class="login_btn_box">
								...
								<input tabindex="4" class="login_btn" name="login" id="kc-login" type="submit" value="${msg('doLogIn')}" />
								<#--  <a href="javascript:void(0);" class="login_btn" role="button" onclick="login(); return false;">로그인</a>  -->
							</div>
							<div class="find_box">
								<a href="javascript:void(0);" <#-- href="/auth/findUserInfo?mode=id" --> role="button" class="find_btn">이메일 찾기</a>
								<#if realm.resetPasswordAllowed>
									<a class="find_btn find_btn_pw" tabindex="5" href="${url.loginResetCredentialsUrl}">${msg("doForgotPassword")}</a>
								</#if>
								<#if realm.password && realm.registrationAllowed && !registrationDisabled??>
									<span>${msg("noAccount")}</span>
									<a class="find_btn find_btn_pw" role="button" tabindex="6" href="${url.registrationUrl}">${msg("doRegister")}</a>
								</#if>
							</div>
						</form>
					</#if>
